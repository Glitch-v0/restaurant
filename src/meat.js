...
export function meatTab (){
    ...
    const introText = "Nice to <b>meat</b> you! We have four mouth-watering "+
    "meaty, grilled cheese selections to choose from.";
    ...
    "Don't wait until Thanksgiving to enjoy this Turkish delight", "Be filled with this Philly"];
    const meat_ids = ["bacon", "chicken", "turkey", "steak"];
    for (let i = 0; i < 4; i++) {
        ec.makeImage(meat_array[i], document.querySelector("#meat-card"+i), "meat-picture", meat_ids[i]);
        ec.styleElementID(meat_ids[i], "src", meat_array[i]);
        ec.makeElement("p", document.querySelector("#meat-card"+i), "meat-text", "meat-text"+i);
